...
export default {
  ...
  methods: {
    async fetchUserData() {
      try {
        const response = await axios.get("https://ggg-n4vt.onrender.com/user", { headers: { token: localStorage.getItem('token') } });
    ...
    async reportCrime() {
      ...
      try {
        const res = await axios.post("https://ggg-n4vt.onrender.com/createCrime", newCrime);
